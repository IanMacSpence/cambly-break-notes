(function() {
  ...
  function loadSettings() {
    try {
      const raw = hasGM ? GM_getValue(SETTINGS_KEY, "{}") : localStorage.getItem(SETTINGS_KEY) || "{}";
  ...
  function getHeaderForEvent(el) {
    ...
    const headerRoot = document.querySelector(".rbc-time-header") || document.querySelector(".rbc-time-content")?.previousElementSibling || document;
  ...
  function supportsFileSystemAccess$1() {
  ...
  function renderNote(el) {
    ...
    if (!noteEl) {
      ...
      const timeLike = el.querySelector(".rbc-event-label") || el.querySelector("[class*='time']") || el.firstElementChild;
...
//# sourceMappingURL=cambly-break-notes.user.js.map
